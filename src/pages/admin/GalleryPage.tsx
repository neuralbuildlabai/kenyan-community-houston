...
export function AdminGalleryPage() {
  ...
  useEffect(() => { loadAlbums(); loadImages() }, [])
  useEffect(() => { loadImages() }, [albumFilter])
  ...
  async function createAlbum() {
    ...
    else { toast.success('Album created'); setNewAlbumName(''); loadAlbums() }
  }
  ...
  async function uploadImage(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    else { toast.success('Image uploaded'); setCaption(''); loadImages() }
    setUploading(false)
  ...
  async function deleteImage() {
    ...
    else { toast.success('Image deleted'); loadImages() }
    setDeleteId(null)
  }
          ...
          <div className="rounded-xl border p-5 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <Button onClick={() => fileRef.current?.click()} disabled={uploading}>
                <Upload className="h-4 w-4 mr-2" />{uploading ? 'Uploading…' : 'Choose & Upload'}
              </Button>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border p-5 space-y-3">
            ...
            <div className="flex gap-2">
              <Input value={newAlbumName} onChange={(e) => setNewAlbumName(e.target.value)} placeholder="New album name" onKeyDown={(e) => e.key === 'Enter' && createAlbum()} />
