...
export function AdminMembersPage() {
  ...
  function exportCsv() {
    const cols = [
      ...
      'membership_type', 'membership_status', 'dues_status', 'submitted_at',
    ]
      ...
      <div className="flex flex-col lg:flex-row gap-3 flex-wrap">
        ...
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          ...
          <SelectContent>
            ...
            {MEM_TYPES.map((t) => <SelectItem key={t} value={t}>{t.replace(/_/g, ' ')}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={memStatusFilter} onValueChange={setMemStatusFilter}>
          <SelectTrigger className="w-full sm:w-44"><SelectValue placeholder="Membership" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All membership</SelectItem>
            {MEM_STATUS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={duesFilter} onValueChange={setDuesFilter}>
          <SelectTrigger className="w-full sm:w-40"><SelectValue placeholder="Dues" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All dues</SelectItem>
            {DUES.map((d) => <SelectItem key={d} value={d}>{d}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">No members match filters</TableCell></TableRow>
            ) : (
              filtered.map((m) => {
                          ...
                          <ul className="space-y-1 text-sm">
                            {hh.map((h) => (
                              <li key={h.id} className="flex flex-wrap gap-x-4 gap-y-1">
                                ...
                                {h.relationship && <Badge variant="outline">{h.relationship}</Badge>}
                                {h.age_group && <span className="text-muted-foreground">{h.age_group}</span>}
                                {h.email && <span className="text-muted-foreground">{h.email}</span>}
